...
Matrix calc_hessian_x(Function f, const State &x, const Action &u, double dt = 0.001)
{
  ...
  for (int i{0}; i < x.size(); ++i)
  {
    for (int j{i}; j < x.size(); ++j)
    {
      ...
      dx_i(i) = dt;
      dx_j(j) = dt;
      H(i, j) = (f(x + dx_i + dx_j, u) - f(x + dx_i - dx_j, u) - f(x - dx_i + dx_j, u) + f(x - dx_i - dx_j, u)) /
                (4.0 * dt * dt);
    }
  }
  return H;
...
Matrix calc_hessian_u(Function f, const State &x, const Action &u, double dt = 0.001)
{
  ...
  for (int i{0}; i < u.size(); ++i)
  {
    for (int j{i}; j < u.size(); ++j)
    {
      ...
      du_j(j) = dt;
      H(i, j) = H(j, i) =
          (f(x, u + du_i + du_j) - f(x, u + du_i - du_j) - f(x, u - du_i + du_j) + f(x, u - du_i - du_j)) /
          (4.0 * dt * dt);
    }
  }
  return H;
...
Matrix calc_hessian_ux(Function f, const State &x, const Action &u, double dt = 0.001)
{
  ...
  for (int i{0}; i < u.size(); ++i)
  {
    for (int j{0}; j < x.size(); ++j)
    {
      ...
      du_i(i) = dt;
      dx_j(j) = dt;
      H(i, j) = (f(x + dx_j, u + du_i) - f(x - dx_j, u + du_i) - f(x + dx_j, u - du_i) + f(x - dx_j, u - du_i)) /
                (4.0 * dt * dt);
    }
  }
  return H;
